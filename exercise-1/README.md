...
3.在下列选择器中，哪一个代表同一个元素同时拥有 section 和 title 两个class? ( class="section title" )
...
A)  .section .title { color:red; }          specificity = 0,0,2,0
B)  #title { color:green; }                 specificity = 0,1,0,0
C)  .title { color:yellow !important; }
D)  .section > h1 { color:blue; }           specificity = 0,0,0,1
```
